...
| Full dependency locks (§22) | Generic Locked Render generation and verification implemented | `maac::generic_lock_normalization` deterministically constructs canonical generic v1 Config, RenderInput, and Lock artifacts from typed caller-resolved context and exact closure bytes; `maac::generic_lock` independently validates and verifies them. `maac::external` additionally discovers and digest-verifies the complete owner-scoped dependency closure for locked external processors. Generic rendering remains separate from §18 Core Audio processor conformance. |
