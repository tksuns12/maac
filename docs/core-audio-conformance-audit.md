...
| Full dependency locks (§22) | Generic Locked Render verifier implemented | `maac::generic_lock` validates the generic v1 lock envelope and verifies caller-resolved execution, dependency, processor, engine, output, and evidence bytes. `maac::external` additionally discovers and digest-verifies the complete owner-scoped dependency closure for locked external processors. Lock generation/normalization and generic rendering remain separate from §18 Core Audio processor conformance. |
