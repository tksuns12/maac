...
| Full dependency locks (§22) | Generic Locked Render verifier implemented | `maac::generic_lock` validates the generic v1 lock envelope and verifies caller-resolved execution, dependency, processor, engine, output, and evidence bytes. Dependency discovery and generic rendering remain separate from §18 Core Audio processor conformance. |
| External processors and adapters (§§17, 25) | Initial interchange adapter implemented; external ABI deferred | `maac::interchange` provides a bounded MIDI 1.0 SMF adapter with versioned machine-readable loss reports and faithful-mode refusal. It explicitly reports microtonal, overlapping-note identity, per-note expression, tempo-ramp, timing-resolution, audio, synthesis/routing, hit-mapping, and unsupported-message losses. Notation/DAW adapters and a general external processor ABI remain deferred. |
