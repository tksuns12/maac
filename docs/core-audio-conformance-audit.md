...
| --- | --- | --- |
| Compact source normalization and revision identity (§20) | Bounded core implementation | [editing](../src/editing/mod.rs) exposes authored canonical revision identity and [FoundationEditContext](../src/editing/foundation.rs) supplies label-retaining normalization for the supported core document slice. [production_identity.rs](../src/production_identity.rs) remains the execution-hash projection. Library/import/extension-aware editing normalization is still outside this bounded context, so this is not full Document-profile conformance. |
| Atomic editing (§21) | Bounded core implementation | `maac::editing` implements all five Protocol 2 operations, immutable-base preconditions, identity correspondence through renames, atomic final validation, inverse patches, source-preserving projection, and bounded impact reporting. `maac patch` exposes atomic file publication for the supported core slice. Library/import/extension-aware contexts and finer dependency/region impact remain follow-up work, so this does not establish full Document-profile editing. |
