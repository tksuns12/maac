...
| 4 | Faithful handoff: MIDI, aligned stems, descriptors, and loss reports | Interchange/profile and adapter contracts | Descriptor fields, latency, state, and fidelity refusal are already specified; wire schemas, receiver mappings, and profiles remain deferred. | A Type 1 MIDI file, aligned PCM WAV stems, original MaaC source/dependencies, and manifest account for overlaps, expression, tempo ramps, audio, and automation, rejecting any unapproved loss. |
