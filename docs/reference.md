...
base revision is checked against the authored typed graph before mutation. The command
loads the source through the same bounded `SourceBundle` resolver used by compilation;
`--project-root` selects the containment root for local imports. Every private candidate
is re-resolved, so edited import paths/hashes are checked for path containment, cycles,
resource limits, built-in identities, and exact SHA-256 pins before commit. Reusable
library sources are valid patch targets, and recognized production-extension/`fx.*`
preconditions share the production execution normalizer. The complete candidate is
semantically validated before publication. `-o` is required; an existing destination is
