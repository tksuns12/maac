...
| Core modulation | Typed control outputs, constant automation and control chains, ID-ordered additive contributions and final range policy, all four LFO waves, score/seconds clocks and their tail behavior; note-on/note-off capture and bounded release work; reset-rate targets, cycles, wrong ports, nonfinite results and precision/resource failures reject explicitly; source-free V7 replay and named delivery |
...
| Internal ADSR modulation | Initial per-note and pre-release source snapshots, ID-ordered capture, held event values, no double advancement, conservative preview/release work, source-free replay; reset targets and cycles reject |
