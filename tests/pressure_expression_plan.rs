...
#[test]
fn pressure_payload_roundtrips() {
...
fn valid_expression() -> PressureExpression {
    PressureExpression {
...
fn with_expression(expression: PressureExpression) -> Plan {
    ...
    *pressure_expression = Some(expression);
    plan
}
fn assert_invalid(expression: PressureExpression, code: &str) {
...
fn invalid_structure_and_negative_values_fail_at_all_boundaries() {
    ...
    for zero_at in [1, 2] {
        ...
        expression.points.push(PressureExpressionPoint {
            position: rat(2, 1),
            value: rat(1, 1),
            shape: Interpolation::Step,
        });
...
#[test]
fn strict_pressure_wire_and_unsupported_expression_fields() {
    let valid = serde_json::to_value(with_expression(valid_expression())).unwrap();
    for field in ["extra", "pressure", "pressure"] {
    ...
    for (field, replacement) in [("clock", json!("beats")), ("extra", json!(true))] {
        let mut bad = valid.clone();
        bad["events"][0]["kind"]["pressure_expression"][field] = replacement;
        assert!(serde_json::from_value::<Plan>(bad).is_err());
    }
    for (field, replacement) in [
        ...
        let mut bad = valid.clone();
        bad["events"][0]["kind"]["pressure_expression"]["points"][0][field] = replacement;
...
fn simultaneous_curves_count_all_points_objects_and_work() {
    let pressure = with_expression(valid_expression());
    let mut both = pressure.clone();
    let EventKind::Note {
        pitch_expression,
        gain_expression,
        timbre_expression,
    ...
    *gain_expression = Some(GainExpression {
        clock: ExpressionClock::Normalized,
        points: valid_expression()
            .points
            .iter()
            .map(|p| GainExpressionPoint {
    ...
    *timbre_expression = Some(TimbreExpression {
    ...
    for objects in [true, false] {
        let first_pass = (0..100)
            ...
            .unwrap();
        for (plan, additional) in [(&pressure, 3), (&both, 12)] {
            let mut limits = PlanLimits::default();
            if objects {
...
#[test]
fn absent_pressure_preserves_legacy_json() {
    let plan = base_plan();
    let bytes = plan.to_json().unwrap();
    assert!(!String::from_utf8(bytes.clone())
        .unwrap()
        .contains("pressure_expression"));
...
#[test]
fn receiver_requires_explicit_pressure_source_even_when_note_is_silent() {
...
#[test]
fn all_four_expression_costs_include_automated_release_even_for_silent_voices() {
    let mut base = base_plan();
    if let EventKind::Note { velocity, .. } = &mut base.events[0].kind {
        *velocity = rat(0, 1);
    }
    base.automation.push(Automation {
        id: "release".into(),
        target: PortRef::new("lead", "release").unwrap(),
        clock: AutomationClock::Seconds,
        at: rat(0, 1),
        points: vec![AutomationPoint {
            position: rat(0, 1),
    ...
    let frames = base.events[0].off_frame.unwrap() - base.events[0].on_frame + 4800;
    let mut plan = base;
    if let EventKind::Note {
        gain_expression,
        pitch_expression,
    ...
    } = &mut plan.events[0].kind
    {
        *timbre_expression = Some(TimbreExpression {
            clock: ExpressionClock::Seconds,
            points: vec![TimbreExpressionPoint {
                position: rat(0, 1),
                value: rat(1, 2),
                shape: Interpolation::Step,
            }],
    ...
    assert_eq!(minimum_execution_budget(&plan), baseline + frames * 51);
    for points in [2usize, 3, 4, 5] {
        ...
        {
            *pressure_expression = Some(PressureExpression {
                clock: ExpressionClock::Seconds,
                points: (0..points)
                    .map(|position| PressureExpressionPoint {
        ...
        assert_eq!(
            ...
            baseline + frames * (51 + 17 + lookup)
        );
        assert_eq!(
            plan.validate_with_limits(&PlanLimits {
                max_automation_points: points + 3,
                ..Default::default()
            })
            .unwrap_err()
            .code,
        ...
        plan.validate_with_limits(&PlanLimits {
            max_automation_points: points + 4,
            ..Default::default()
        })
        .unwrap();
    }
...
#[test]
fn pressure_and_timbre_capabilities_are_independent() {
    let mut plan = base_plan();
    plan.instruments.as_mut().unwrap().programs[0]
        .voice
        .nodes
        .retain(|node| node.id != "timbre");
    if let EventKind::Note {
        timbre_expression, ..
    } = &mut plan.events[0].kind
    {
    ...
    assert!(error.path.contains("timbre_expression"));
    let mut plan = with_expression(valid_expression());
    plan.instruments.as_mut().unwrap().programs[0]
        .voice
        .nodes
    ...
    let error = plan.validate().unwrap_err();
    assert_eq!(error.code, "E_CAPABILITY");
    assert!(error.path.contains("pressure_expression"));
}
