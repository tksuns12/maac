...
instrument lead {{ channels = 1; voice v {{ channels = 1; amplitude = &amp; output = &color:out; node amp {{ type = "synth.adsr/1"; }} node color {{ type = "synth.pressure/1"; }} node shade {{ type = "synth.timbre/1"; }} }} }}
...
#[test]
fn normalized_pressure_compiles() {
    let source = base(
        r#"curve bend { clock = normalized; points = [(0, 0, linear), (1, 1, step)]; }
 pattern phrase { length = 1q; note n { at = 0q; dur = 1q; pitch = A4; expression e { kind = pressure; curve = &bend; } } }
 place p1 { pattern = &phrase; track = &t; at = 0q; }"#,
        "4q",
    );
    let plan = compile(&parse(&source).unwrap()).expect("pressure expression compiles");
    plan.validate().unwrap();
    assert!(matches!(
        &plan.events[0].kind,
        EventKind::Note {
            pressure_expression: Some(_),
            ..
        }
    ));
}
...
pattern phrase {{ length = 1q; note n {{ at = 0q; dur = 2q; pitch = A4; expression e {{ kind = pressure; curve = &shade; }} }} }}
...
insert extra {{ note added {{ at = 2q; dur = 1q; pitch = A4; expression e {{ kind = pressure; curve = &shade; }} }} }} }}
...
expression z { kind = pressure; curve = &shade; } expression touch { kind = timbre; curve = &shade; } expression a { kind = gain; curve = &shade; } expression m { kind = pitch; curve = &bend; } } }
        ...
        if limit == 15 {
            assert!(result
                ...
                .any(|e| e.code == DiagnosticCode::ResourceLimit
                    && e.message == "aggregate automation and expression point budget exceeded"));
        } else {
...
fn invalid_values_units_and_receivers_fail() {
    let text = base(
        r#"curve shade { clock = normalized; points = [(0, 0, linear), (1, 1, step)]; }
pattern phrase { length = 1q; note n { at = 0q; dur = 1q; pitch = A4; expression e { kind = pressure; curve = &shade; } } }
place p1 { pattern = &phrase; track = &t; at = 0q; }"#,
        "4q",
    );
    for (bad, code) in [
        (text.replace("expression e { kind = pressure; curve = &shade; }", "expression e { kind = pressure; curve = &shade; } expression duplicate { kind = pressure; curve = &shade; }"), DiagnosticCode::Range),
        ...
        (
            text.replace("synth.pressure/1", "synth.sine/1"),
        ...
        (
            text.replace("synth.pressure/1", "synth.timbre/1"),
            DiagnosticCode::Capability,
        ),
    ] {
