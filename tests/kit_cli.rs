...
node kit {{ type="core.kit/1"; config={{channels=1; voices=2; samples=[{{key="key"; asset=&sample;}}];}}; }}
track track {{ target=&kit:events; }}
pattern phrase {{ length=1/3000q; hit a {{ at=0q; key="key"; }} hit b {{ at=1/12000q; key="key"; }} }}
...
#[test]
fn additive_command_api_preserves_legacy_json_and_reports_real_mixed_counts() {
    use maac::cli::{
        execute, execute_artifact, format_human, format_human_artifact, Command as CliCommand,
        ProfileArg,
    };
    let d = tempdir().unwrap();
    ...
    assert_eq!(execute(&command).unwrap_err().code, "E_CAPABILITY");
    let mut text = fs::read_to_string(&input).unwrap();
    text.push_str(
        r#"
node sine { type="core.sine/1"; }
track notes { target=&sine:events; }
pattern note_pattern { length=1/3000q; note n { at=0q; dur=1/24000q; pitch=A4; } }
place note_place { pattern=&note_pattern; track=&notes; at=0q; }
"#,
    );
    fs::write(&input, text).unwrap();
    let result = execute_artifact(&command).unwrap();
    assert_eq!(result.base().notes, Some(1));
    assert_eq!(result.hits(), 2);
    assert!(format_human_artifact(&result).contains("1 notes, 2 hits"));
    let human = invoke(&[
        Path::new("check"),
        &input,
        Path::new("--project-root"),
    ...
    assert!(String::from_utf8(human.stdout)
        .unwrap()
        .contains("1 notes, 2 hits"));
    let legacy = d.path().join("legacy.maac");
    fs::write(
        &legacy,
        r#"maac 1;
project p { score=[0q,1/3000q]; rate=48000Hz; tempo=&t; meter=&m; output=&s:out; }
...
"#,
    ...
    let old = execute(&command).unwrap();
    let new = execute_artifact(&command).unwrap();
    assert_eq!(new.hits(), 0);
    assert_eq!(
        serde_json::to_vec(&old).unwrap(),
        serde_json::to_vec(&new).unwrap()
    );
    assert_eq!(format_human(&old), format_human_artifact(&new));
}
...
fn artifact_export_preparation_budget_failure_leaves_sink_empty() {
    ...
    artifact.validate_with_limits(&limits).unwrap();
...
#[test]
fn deliver_routes_source_and_retained_kit_artifacts() {
    let d = tempdir().unwrap();
    let root = d.path();
    let (input, asset) = setup(root, &[0.25, 0.5]);
    ...
    let mut text = fs::read_to_string(&input).unwrap().replace(
        "output=&kit:out;",
        "output=&kit:out; requires=[\"maac.production/1\"];",
    );
    text.push_str(&format!(r#"
asset schema {{kind=descriptor;path="production.schema.json";hash="{}";}}
extension deliveries {{namespace="maac.production/1";schema=&schema;render_affecting=true;data={{deliveries={{release={{rate=48000Hz;resampler="maac.src.kaiser/1";targets={{master={{role=master;output=&kit:out;encoding=wav_f32le;dither={{type=none;}};}};}};}};}};}};}}
