...
const EXAMPLE: &str = include_str!("../examples/internal-reset-modulation.maac");
...
fn assert_first_float_sample(path: &Path, expected: f64) {
    let mut reader = WavReader::open(path).unwrap();
    assert_eq!(reader.spec().sample_format, SampleFormat::Float);
    let actual = f64::from(reader.samples::<f32>().next().unwrap().unwrap());
    assert!(
        (actual - expected).abs() < 1.0e-6,
        "first sample {actual} != {expected}"
    );
}

#[test]
fn internal_reset_example_roundtrips_v2_plan_and_captures_before_frame0_automation() {
    let directory = tempdir().unwrap();
    let cwd = directory.path();
    let source = cwd.join("internal-reset.maac");
    let plan = cwd.join("internal-reset.json");
    fs::write(&source, EXAMPLE).unwrap();

    succeeds(cwd, &["check", "internal-reset.maac"]);
    succeeds(
        cwd,
        &[
            "compile",
    ...
    let shared = &encoded["instruments"]["programs"][0]["shared"];
    let modulations = shared["modulations"].as_array().unwrap();
    assert_eq!(modulations.len(), 2);
    assert!(modulations.iter().any(|edge| {
        edge["id"] == "reset_phase"
            && edge["from"]["node"] == "source"
            && edge["to"] == serde_json::json!({"node": "target", "parameter": "phase"})
    }));
    assert!(modulations.iter().any(|edge| {
        edge["id"] == "source_level"
            && edge["to"] == serde_json::json!({"node": "gain", "parameter": "level"})
    }));
    assert_eq!(
        encoded["automation"][0]["target"],
        serde_json::json!({"node": "sound", "port": "source_level"})
    );
    let instance = encoded["nodes"]
        ...
        .unwrap();
    assert_eq!(instance["params"]["source_phase"], "1/4");
    assert_eq!(instance["params"]["source_level"], "1/2");
    assert_eq!(instance["params"]["target_phase"], "0/1");
    ...
    assert_first_float_sample(&cwd.join("build-float32.wav"), 3.0 / 16.0);
...
fn assert_reset_failure_preserves_output(
    ...
    let original = b"existing output must survive internal reset failure";
    if exists {
...
#[test]
fn invalid_reset_phase_keeps_build_and_render_outputs_atomic_with_force() {
    let directory = tempdir().unwrap();
    let cwd = directory.path();
    let invalid_source = EXAMPLE.replace(
        "params = { target_phase = 0; };",
        "params = { target_phase = 1; };",
    );
