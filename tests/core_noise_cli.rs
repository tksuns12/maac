use hound::{SampleFormat, WavReader};
use serde_json::Value;
use std::fs;
use std::path::Path;
use std::process::{Command, Output};
use tempfile::tempdir;

const EXAMPLE: &str = include_str!("../examples/core-noise.maac");

fn invoke(directory: &Path, args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_maac"))
        .current_dir(directory)
        .args(args)
        .output()
        .expect("maac starts")
}

fn succeeds(directory: &Path, args: &[&str]) {
    let output = invoke(directory, args);
    assert!(output.status.success(), "{args:?}: {output:?}");
}

fn assert_audio(path: &Path, format: &str) {
    let mut reader = WavReader::open(path).unwrap();
    assert_eq!(reader.duration(), 12_000);
    assert_eq!(reader.spec().sample_rate, 48_000);
    assert_eq!(reader.spec().channels, 2);
    let samples = match format {
        "float32" => {
            assert_eq!(reader.spec().sample_format, SampleFormat::Float);
            assert_eq!(reader.spec().bits_per_sample, 32);
            reader
                .samples::<f32>()
                .map(|sample| {
                    let sample = sample.unwrap();
                    assert!(sample.is_finite());
                    f64::from(sample)
                })
                .collect::<Vec<_>>()
        }
        "pcm16" => {
            assert_eq!(reader.spec().sample_format, SampleFormat::Int);
            assert_eq!(reader.spec().bits_per_sample, 16);
            reader
                .samples::<i16>()
                .map(|sample| f64::from(sample.unwrap()) / 32_768.0)
                .collect::<Vec<_>>()
        }
        _ => panic!("unknown format {format}"),
    };
    let peak = samples
        .iter()
        .map(|sample| sample.abs())
        .fold(0.0, f64::max);
    let left_peak = samples
        .iter()
        .step_by(2)
        .map(|sample| sample.abs())
        .fold(0.0, f64::max);
    let right_peak = samples
        .iter()
        .skip(1)
        .step_by(2)
        .map(|sample| sample.abs())
        .fold(0.0, f64::max);
    assert!(peak > 0.0, "{format} render is silent");
    assert!(peak < 1.0, "{format} peak is out of range: {peak}");
    assert!(
        left_peak > 0.0 && right_peak > 0.0,
        "{format} channel is silent"
    );
}

#[test]
fn noise_example_compiles_and_replays_without_source_in_both_wav_formats() {
    let directory = tempdir().unwrap();
    let cwd = directory.path();
    let source = cwd.join("core-noise.maac");
    let plan = cwd.join("core-noise.json");
    fs::write(&source, EXAMPLE).unwrap();

    succeeds(cwd, &["check", "core-noise.maac"]);
    succeeds(
        cwd,
        &["compile", "core-noise.maac", "-o", "core-noise.json"],
    );

    let encoded: Value = serde_json::from_slice(&fs::read(&plan).unwrap()).unwrap();
    let noise = encoded["nodes"]
        .as_array()
        .unwrap()
        .iter()
        .find(|node| node["id"] == "noise")
        .unwrap();
    assert_eq!(
        noise["processor"],
        serde_json::json!({"kind": "noise", "channels": 2, "seed": 7})
    );
    assert_eq!(noise["params"], serde_json::json!({}));

    for format in ["float32", "pcm16"] {
        let output = format!("build-{format}.wav");
        succeeds(
            cwd,
            &[
                "build",
                "core-noise.maac",
                "-o",
                output.as_str(),
                "--format",
                format,
            ],
        );
        assert_audio(&cwd.join(&output), format);
    }

    fs::remove_file(&source).unwrap();
    assert!(!source.exists());
    let empty = tempdir().unwrap();
    let plan_arg = plan.to_str().unwrap();
    for format in ["float32", "pcm16"] {
        let expected = fs::read(cwd.join(format!("build-{format}.wav"))).unwrap();
        let rendered = cwd.join(format!("render-{format}.wav"));
        succeeds(
            empty.path(),
            &[
                "render",
                plan_arg,
                "-o",
                rendered.to_str().unwrap(),
                "--format",
                format,
            ],
        );
        assert_eq!(fs::read(&rendered).unwrap(), expected);
        assert_audio(&rendered, format);
    }
}

fn assert_noise_failure_preserves_output(
    cwd: &Path,
    args: &[&str],
    output_name: &str,
    expected_code: &str,
    exists: bool,
) {
    let output_path = cwd.join(output_name);
    let original = b"existing output must survive noise source failure";
    if exists {
        fs::write(&output_path, original).unwrap();
    }
    let output = invoke(cwd, args);
    assert!(!output.status.success(), "{args:?}: {output:?}");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains(expected_code), "{stderr}");
    if exists {
        assert_eq!(fs::read(&output_path).unwrap(), original);
    } else {
        assert!(!output_path.exists());
    }
}

#[test]
fn invalid_project_seed_and_noise_parameters_keep_forced_outputs_atomic() {
    let directory = tempdir().unwrap();
    let cwd = directory.path();
    let invalid_parameters = EXAMPLE.replace(
        "  config = { channels = 2; };\n}\nnode attenuate",
        "  config = { channels = 2; };\n  params = { level = 1; };\n}\nnode attenuate",
    );
    assert_ne!(invalid_parameters, EXAMPLE);
    for (label, source, expected_code) in [
        (
            "seed",
            EXAMPLE.replace("  seed = 7;", "  seed = -1;"),
            "E_RANGE",
        ),
        ("params", invalid_parameters, "E_UNKNOWN_FIELD"),
    ] {
        fs::write(cwd.join("invalid.maac"), source).unwrap();
        for exists in [true, false] {
            let output_name = format!(
                "{label}-{}-output.wav",
                if exists { "existing" } else { "absent" }
            );
            assert_noise_failure_preserves_output(
                cwd,
                &[
                    "build",
                    "invalid.maac",
                    "-o",
                    output_name.as_str(),
                    "--force",
                ],
                &output_name,
                expected_code,
                exists,
            );
        }
    }
}
